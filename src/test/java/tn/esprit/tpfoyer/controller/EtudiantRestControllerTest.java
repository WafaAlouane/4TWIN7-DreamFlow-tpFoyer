...
class EtudiantRestControllerTest {
    ...
    void testAddEtudiantDuplicateCin() throws Exception {
        when(etudiantService.recupererEtudiantParCin(Long.parseLong("12345678"))).thenReturn(new Etudiant());
    ...
    void testRetrieveEtudiantByCin() throws Exception {
        ...
        when(etudiantService.recupererEtudiantParCin(Long.parseLong("12345678"))).thenReturn(etudiant);

        mockMvc.perform(get("/etudiant/retrieve-etudiant-cin/12345678"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.cinEtudiant").value("12345678"));

        verify(etudiantService, times(1)).recupererEtudiantParCin(Long.parseLong("12345678"));
    }

    @Test
    void testRetrieveEtudiantByCinNotFound() throws Exception {
        when(etudiantService.recupererEtudiantParCin(Long.parseLong("12345678"))).thenReturn(null);
